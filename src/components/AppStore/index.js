...
class AppStore extends Component {
  state = {categoryName: tabsList[0].tabId, search: ''}

  categoryApps = () => {
    const {categoryName} = this.state

    const filteredList = appsList.filter(
      eachApp => eachApp.category === categoryName,
    )
  ...
  searchedResults = () => {
    ...
    const filteredList = this.categoryApps()

    const searchedList = filteredList.filter(eachApp =>
  ...
  render() {
    ...
    const searchedList = this.searchedResults()
    return (
      <div className="app-store-container">
        <div>
          <h1 className="title">App Store</h1>
          <div className="total-search-tab-container">
            <input
              type="search"
              placeholder="search"
              className="just-search-bar"
              value={search}
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === categoryName}
              />
            ))}
          </ul>
